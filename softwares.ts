...
const warning = colors.bgYellow.bold
const error = colors.red

export async function installDockerEngine() {
	console.log(info('installing docker engine...'))
...
export async function dockerEnginePostInstall() {
	console.log(info('processing docker engine post-install...'))
	...
	console.log(success('docker post-install processed ✔\n'))
	!(await isCurrentUserInDockerGroup()) && console.log(warning('reopen your terminal to use docker without sudo!'))
}

export async function installOhMyZsh() {
	console.log(info('installing ohmyzsh...'))
	...
	console.log(warning('run "chsh -s $(which zsh)" to set as a default shell!'))
	console.log(success('ohmyzsh installed ✔\n'))
}

async function installZshPlugin(plugin: string) {
	const path = `/home/$USER/.oh-my-zsh/custom/plugins/${plugin}`
	if (!await exists(path)) {
		console.log(info(`installing ohmyzsh plugin ${plugin}...`))
		await bash(`git clone https://github.com/zsh-users/${plugin} ${path}`)
		console.log(success(`ohmyzsh plugin ${plugin} installed ✔\n`))
	} else console.log(warning(`${plugin} already installed`))
}

export async function installDeno() {
	console.log(info('installing deno...'))
	...
	console.log(success('deno installed ✔\n'))
}

export async function installStarship() {
	console.log(info('installing starship...'))
	...
	console.log(success('starship installed ✔\n'))
}

export async function installMkcert() {
	console.log(info('installing mkcert...'))
	...
	console.log(success('mkcert installed ✔\n'))
}
...
export async function installCarootOnWsl(path: string, target: string) {
	...
	console.log(success(`mkcert root certs successfully installed in ${target} ✔`))
}

export async function installCtop() {
	// https://github.com/bcicen/ctop
	console.log(info('installing ctop...'))
...
export async function installSshs() {
	...
	console.log(success(`sshs installed ✔\n`))
}

export async function installBottom() {
	// https://github.com/ClementTsang/bottom
	console.log(info('installing bottom...'))
	await run(`wget https://github.com/ClementTsang/bottom/releases/download/0.14.2/bottom_0.14.2-1_amd64.deb -O ./bottom_amd64.deb`.split(' '))
...
export async function installMagentoCloudCli() {
	console.log(info('installing magento-cloud-cli...'))
	if (!await isInstalled('php')) return console.log(error('magento-cloud-cli: php is required!'))
	let cmd = `curl -sS https://accounts.magento.cloud/cli/installer | php`
	if (await isInstalled('zsh')) cmd += ' -- --shell-type zsh'
	await run(['sh', '-c', cmd])
	console.log(success('magento-cloud-cli installed ✔\n'))
}

export async function installMage2Postman() {
	console.log(info('installing mage2postman...'))
	...
	console.log(success('mage2postman installed ✔\n'))
}

export async function installEnvman(version: string) {
	console.log(info('updating evnman...'))
...
export async function installJetBrainsGateway() {
	...
	if (
		...
		console.log(success(`JetBrains Gateway (${newVersion}) installed ✔\n`))
	}
}

export async function installSig() {
	// https://github.com/ynqa/sig
	console.log(info('installing sig...'))
	...
	console.log(success('sig installed ✔\n'))
}

export async function installLazygit() {
	// https://github.com/jesseduffield/lazygit
	console.log(info('installing lazygit...'))
	await run(`wget https://github.com/jesseduffield/lazygit/releases/download/v0.62.2/lazygit_0.62.2_linux_32-bit.tar.gz -O lazygit.tar.gz`.split(' '))
	await run('mkdir -p tmp && tar -xf lazygit.tar.gz -C tmp'.split(' '))
	await run('sudo install tmp/lazygit /usr/local/bin'.split(' '))
	await run('rm -rf tmp lazygit.tar.gz'.split(' '))
	console.log(success('lazygit installed ✔\n'))
}

export async function installFnm() {
	// https://github.com/Schniz/fnm
	console.log(info('installing fnm...'))
	await run('sudo apt install unzip'.split(' '))
	await run(['sh', '-c', 'curl -fsSL https://fnm.vercel.app/install | bash'])
	console.log(warning('restart your terminal to use fnm!'))
	console.log(success(`fnm installed ✔\n`))
}

export async function installOllama() {
	// https://github.com/ollama/ollama
	console.log(info('installing Ollama...'))
